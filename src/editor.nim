...
proc processEditor*() =
  #draw palette
  let 
    minSize = 60f #minimum size, disregarding rows
    pad = 10f #padding on left and right of screen
    #maxRows = 3 #maximum palette rows
  ...
  for i, color in curPalette.colors:
    let bounds = rect((i mod colorsPerRow).float32 * psize + xoff, fau.heightf - psize - row*psize, psize, psize)
    if button(
        bounds, 
    ...
    if curColor == color:
      lineRect(bounds.x, bounds.y, bounds.w, bounds.h, stroke = 10f, color = colorCoral, z = 1f)
